...
fn move_ball_position(ball_position: &mut PixelAddr, ballvelocity: &BallVelocity, lines_vec: &mut Vec<String>) {
    change_char(ball_position.clone(), ' ', lines_vec);
    match ballvelocity.direction {
        5 => ball_position.y += ballvelocity.speed,
        2 => {ball_position.y -= ballvelocity.speed; ball_position.x += ballvelocity.speed;},
        3 => ball_position.x += ballvelocity.speed,
        4 => {ball_position.y += ballvelocity.speed; ball_position.x += ballvelocity.speed;},
        7 => ball_position.x -= ballvelocity.speed,
        8 => {ball_position.y -= ballvelocity.speed; ball_position.x -= ballvelocity.speed;},
        1 => ball_position.y -= ballvelocity.speed,
        6 => {ball_position.y += ballvelocity.speed; ball_position.x -= ballvelocity.speed;},
        _ => panic!("Velocit direction invalid")
    }
    change_char(ball_position.clone(), '◯', lines_vec);
}
...
fn render_bot_platform(bot_position: &mut i32, ball_position: PixelAddr, lines_vec: &mut Vec<String>) {
    if ball_position.y == 0 {
        *bot_position = 0;
    }else if ball_position.y == 9 {
        *bot_position = 8;
    }else {
    ...
    for i in 0..(lines_vec.len() as i32) {
        if i >= *bot_position && i <= *bot_position + 3 {
            change_char(PixelAddr { x: 48, y: i }, '█', lines_vec);
        }else {
            change_char(PixelAddr { x: 48, y: i }, ' ', lines_vec);
        }
    }
...
fn run() -> io::Result<()>{
    ...
    let dimensions = GroundDimension{width: 50, height: 10};
    ...
    let mut ballvelocity = BallVelocity{ speed: 1, direction: 4};
    ...
    loop {
            ...
            if let Event::Key(key) = event::read()? {
                if key.kind == KeyEventKind::Press {
                    match key.code {
                        ...
                        KeyCode::Up => if platform_position > 0 {platform_position -= 1},
                        KeyCode::Down =>if platform_position + 4 < dimensions.height {platform_position += 1},
                        _ => (),
                    }
                }
            }
        ...
        if ball_position.x == 47 && ballvelocity.direction < 3{
            ballvelocity.direction += 6;
        }else if ball_position.x ==47 && ballvelocity.direction > 3 {
            ballvelocity.direction += 2;
        }
        if ball_position.x == 1 && ballvelocity.direction < 7 
            && ball_position.y >= platform_position && ball_position.y <= platform_position + 3{
                ballvelocity.direction -= 2;
            }else if ball_position.x == 1 && ballvelocity.direction > 7 
                && ball_position.y >= platform_position && ball_position.y <= platform_position + 3{
                    ballvelocity.direction -= 6;
            }
            print!("\x1b[14F\x1b[J");
